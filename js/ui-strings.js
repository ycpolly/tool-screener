...
const UI_STRINGS = {
  ...
  FAILURE_REASONS: {
    ...
    KD_OUT_OF_RANGE: (kVal, minK = 30, maxK = 65) => `KD數值不符 (K:${kVal} 不在 ${minK}-${maxK})`,
    KD_DEATH_CROSS: (kVal, dVal) => `KD死亡交叉 (K:${kVal} < D:${dVal})`,
    KD_OVERHEAT: (kVal) => `KD過熱排除 (K:${kVal} > 80)`,
    KD_LOW_ENTRY: (kVal, minK = 30, maxK = 65) => `KD未達標 (K:${kVal} 不在 ${minK}-${maxK})`,
    KD_MOMENTUM: (kVal, dVal) => (dVal ? `KD未達高檔區 (K:${kVal} < D:${dVal})` : `KD未達高檔區 (K:${kVal} < 65)`),
    CANDLE_MOMENTUM: (shadowRatio) => shadowRatio ? `避雷針上影線 (上影線為實體 ${shadowRatio.toFixed(1)}倍)` : '避雷針上影線',
    CANDLE_LOW_ENTRY: (ratioPct) => ratioPct ? `長黑倒貨K棒 (開高走低摔 ${ratioPct.toFixed(2)}%)` : '長黑倒貨K棒',
